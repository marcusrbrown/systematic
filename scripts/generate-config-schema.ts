...
function main(): void {
  ...
  const schemaContent = generateSchemaContent(resolvedVersion)
  ...
  // Regenerate bundled-names.ts in lockstep. Sanity check aborts on empty
  // discovery or shrinkage (unless --allow-shrink is passed for an intentional
  // bundled-agent or bundled-skill removal). CI never passes --allow-shrink.
  let bundledNamesPath: string
  try {
    const { agents, skills } = discoverBundledNames(PROJECT_ROOT)
