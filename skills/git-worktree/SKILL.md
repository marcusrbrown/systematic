...
1. **Code Review (`/systematic:ce-review`)**: If NOT already on the target branch (PR branch or requested branch), offer worktree for isolated review
2. **Feature Work (`/systematic:ce-work`)**: Always ask if user wants parallel worktree or live branch work
...
The skill is automatically called from `/systematic:ce-review` and `/systematic:ce-work` commands:
