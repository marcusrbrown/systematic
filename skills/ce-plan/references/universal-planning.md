...
- **Pipeline mode?** If invoked from LFG or any `disable-model-invocation` context: output "This is a non-software task. The LFG pipeline requires ce-work, which only supports software tasks. Use `systematic:ce-plan` directly for non-software planning." and stop.
...
When research is recommended, do it — don't just offer. Stale recommendations (closed restaurants, rethemed attractions, outdated prices) are worse than no recommendations. The user invoked `systematic:ce-plan` because they want a good plan, not a disclaimer about training data.
...
Do not offer `systematic:ce-work` (software-only) or issue creation (not applicable to non-software plans).
