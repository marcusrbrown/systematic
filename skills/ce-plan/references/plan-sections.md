...
- **The implementing agent** (`systematic:ce-work` or a human) starts from an informed
...
Not every invocation of `systematic:ce-plan` should produce a plan document. For
genuinely atomic work, the doc is ceremony — the implementer (whether
`systematic:ce-work` or a human) can act directly without IDed units, KTDs, or
...
When skipping the plan doc, the work proceeds directly to `systematic:ce-work` or to
...
- **`status`** — `active` on creation; `systematic:ce-work` flips to `completed` on
  ship. `systematic:ce-plan`'s Phase 0.1 resume fast path keys on `active`.
...
- **`origin`** — repo-relative path to an upstream brainstorm requirements
  ...
  and re-resolved when `systematic:ce-plan` re-deepens.
