...
This content is loaded when a synthesis-summary phase fires in systematic:ce-plan. There are two variants — they share structure but differ in timing and content focus:
...
- AND Phase 0.4 stayed in systematic:ce-plan (did not route to systematic:ce-work or universal-planning)
...
**Why pre-research, not pre-write**: research effort would be wasted if scope is wrong. Catching scope errors before sub-agent dispatch (Phase 1.1's systematic:repo-research-analyst, systematic:learnings-researcher, etc.) saves token and time cost.
...
**Reads from doc body, not a synthesis section**: brainstorm docs do not have a `## Synthesis` section (the synthesis is a chat-time artifact in systematic:ce-brainstorm; only the prose summary embeds, as `## Summary`). Phase 5.1.5 derives plan-time decisions from the brainstorm doc's body sections — Summary, Problem Frame, Requirements, Key Decisions, Scope Boundaries — plus Phase 1 research.
...
When the skill is invoked from an automated workflow such as LFG or any `disable-model-invocation` context, the skill runs in non-interactive mode (no synchronous user). The artifact is read by downstream skills (document-review, systematic:ce-work) and human reviewers (PR review).
...
This restores the audit visibility the original design intended (un-validated bets must not propagate as authoritative content), but surfaces them under their own label rather than hiding them. Downstream review (document-review, systematic:ce-work, human PR review) can scrutinize Assumptions specifically.
...
- **Solo variant**: common redirects include "this is bigger than I thought — let me brainstorm first" (suggest `systematic:ce-brainstorm`), "this is just a fix, no plan needed" (suggest `systematic:ce-work`), or "I need to investigate first" (route to root-cause investigation before planning).
- **Brainstorm-sourced variant**: less common, but possible — "actually this scope is wrong, take it back to brainstorm" (suggest `systematic:ce-brainstorm` to revise the upstream doc).

In either case: stop systematic:ce-plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
