...
**Options:**
1. **Start `systematic:ce-work`** (recommended) - Begin implementing this plan in the current session
...
Based on selection:
- **Start `systematic:ce-work`** -> Call `systematic:ce-work` with the plan path
...
- Ask whether to proceed to `systematic:ce-work` using the platform's blocking question tool
