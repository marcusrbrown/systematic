...
`systematic:ce-plan` does the first planning pass. `deepen-plan` is a second-pass confidence check.
...
6. **Respect product boundaries** - Do not invent new product requirements. If deepening reveals a product-level gap, surface it as an open question or route back to `systematic:ce-brainstorm`.
...
### Phase 1: Parse the Current `systematic:ce-plan` Structure
...
- `systematic:spec-flow-analyzer` for missing user flows, edge cases, and handoff gaps
- `systematic:repo-research-analyst` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `systematic:learnings-researcher` for institutional knowledge and past solved problems
- `systematic:framework-docs-researcher` for official framework or library behavior
- `systematic:best-practices-researcher` for current external patterns and industry guidance
- Add `systematic:git-history-analyzer` only when historical rationale or prior art is materially missing

**Key Technical Decisions**
- `systematic:architecture-strategist` for design integrity, boundaries, and architectural tradeoffs
- Add `systematic:framework-docs-researcher` or `systematic:best-practices-researcher` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `systematic:architecture-strategist` for validating that the technical design accurately represents the intended approach and identifying gaps
- `systematic:repo-research-analyst` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `systematic:best-practices-researcher` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `systematic:repo-research-analyst` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues
- `systematic:pattern-recognition-specialist` for consistency, duplication risks, and alignment with existing patterns
- Add `systematic:spec-flow-analyzer` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `systematic:architecture-strategist` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `systematic:performance-reviewer` for scalability, latency, throughput, and resource-risk analysis
  - `systematic:security-reviewer` for auth, validation, exploit surfaces, and security boundary review
  - `systematic:data-migrations-reviewer` for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `systematic:security-reviewer` for security, auth, privacy, and exploit risk
  - `systematic:data-migrations-reviewer` for persistent data safety, constraints, transaction boundaries, and migration realism
  - `systematic:deployment-verification-agent` for rollout checklists, rollback planning, and launch verification
  - `systematic:performance-reviewer` for capacity, latency, and scaling concerns
...
- Recommend `systematic:ce-brainstorm` if the gap is truly product-defining
...
3. **Start `systematic:ce-work` skill** - Begin implementing the plan
...
- **Start `systematic:ce-work` skill** -> Call the `systematic:ce-work` skill with the plan path
