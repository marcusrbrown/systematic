...
1. **Read Plan and Clarify** _(skip if arriving from Phase 0 with a bare prompt)_
   ...
   - If the plan has a `Prior-Art Survey` section, validate it before environment setup. A missing section means the plan predates this contract, so proceed without rejecting the plan. For a present section, stop before setup when any of these conditions applies:
     1. **Empty survey:** the section exists but contains no survey result. Resolve it by rerunning planning and recording the structured survey.
     2. **Unaccepted failure verdict:** the verdict is `unscoped` or `unresolved` and the section has no recorded user acceptance for that verdict. Resolve it by narrowing the scope, increasing the bounded budget, or getting the user to accept the uncertainty and recording that acceptance.
     3. **Stale surveyed scope:** the surveyed scope has changed since the survey ran. Check only the workspace or subtree recorded by the survey, not the whole repository. Use available VCS history or diff data when present; otherwise compare the scope against any recorded baseline, manifest, timestamps, or other portable local evidence. If history is unavailable, degrade gracefully rather than assuming a specific VCS or failing opaquely: report that freshness could not be established and ask for a fresh survey (or an explicit review of the recorded baseline). Resolve a stale result by rerunning the survey against the current scope and updating the plan.
