...
#### 1. **Context Analyzer** — `systematic:repo-research-analyst`
...
#### 2. **Solution Extractor** — `systematic:repo-research-analyst`
...
#### 3. **Related Docs Finder** — `systematic:learnings-researcher`
...
`systematic:ce-compound-refresh` is **not** a default follow-up. Use it selectively when the new learning suggests an older learning or pattern doc may now be inaccurate.

It makes sense to invoke `systematic:ce-compound-refresh` when one or more of these are true:
...
It does **not** make sense to invoke `systematic:ce-compound-refresh` when:
...
- If there is **one obvious stale candidate**, invoke `systematic:ce-compound-refresh` with a narrow scope hint after the new learning is written
- If there are **multiple candidates in the same area**, ask the user whether to run a targeted refresh for that module, category, or pattern set
- If context is already tight or you are in lightweight mode, do not expand into a broad refresh automatically; instead recommend `systematic:ce-compound-refresh` as the next step with a scope hint

When invoking or recommending `systematic:ce-compound-refresh`, be explicit about the argument to pass. Prefer the narrowest useful scope:
...
Do not invoke `systematic:ce-compound-refresh` without an argument unless the user explicitly wants a broad sweep.
...
- **performance_issue** → `systematic:performance-reviewer`
- **security_issue** → `systematic:security-reviewer`
- **database_issue** → `systematic:data-migrations-reviewer`
- Any code-heavy issue → always run `systematic:code-simplicity-reviewer`, and additionally run the kieran reviewer that matches the repo's primary stack:
  - TypeScript/JavaScript → also run `systematic:kieran-typescript-reviewer`
...
In lightweight mode, the overlap check is skipped (no Related Docs Finder subagent). This means lightweight mode may create a doc that overlaps with an existing one. That is acceptable — `systematic:ce-compound-refresh` will catch it later. Only suggest `systematic:ce-compound-refresh` if there is an obvious narrow refresh target. Do not broaden into a large refresh sweep from a lightweight session.
...
### Code Quality & Review
- **systematic:kieran-typescript-reviewer**: Reviews code examples for TypeScript best practices
- **systematic:code-simplicity-reviewer**: Ensures solution code is minimal and clear
- **systematic:pattern-recognition-specialist**: Identifies anti-patterns or repeating issues

### Specific Domain Experts
- **systematic:performance-reviewer**: Analyzes performance_issue category solutions
- **systematic:security-reviewer**: Reviews security_issue solutions for vulnerabilities
- **systematic:data-migrations-reviewer**: Reviews database_issue migrations and queries

### Enhancement & Research
- **systematic:best-practices-researcher**: Enriches solution with industry best practices
- **systematic:framework-docs-researcher**: Links to framework/library documentation references
