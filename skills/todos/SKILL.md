...
| Trigger | Flow |
|---------|------|
| Code review | `/systematic:ce-review` -> Findings -> `/systematic:todos` (Triage section) -> Todos |
| Autonomous review | `/systematic:ce-review mode:autofix` -> Residual todos -> `/systematic:todos` (Resolve section) |
...
Residual actionable work from `systematic:ce-review mode:autofix` after its `safe_auto` pass will already be `ready`.
...
Spawn a `systematic:pr-comment-resolver` agent per item. Prefer parallel; fall back to sequential respecting dependency order.
...
Load the `systematic:ce-compound` skill to document what was learned. Todo resolutions often surface patterns and architectural insights worth capturing.
