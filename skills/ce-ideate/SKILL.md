...
`systematic:ce-ideate` precedes `systematic:ce-brainstorm`.

- `systematic:ce-ideate` answers: "What are the strongest ideas worth exploring?"
- `systematic:ce-brainstorm` answers: "What exactly should one chosen idea mean?"
- `systematic:ce-plan` answers: "How should it be built?"
...
3. **Route action into brainstorming** - Ideation identifies promising directions; `systematic:ce-brainstorm` defines the selected one precisely enough for planning. Do not skip to planning from ideation output.
...
1. **Quick context scan** — dispatch `systematic:repo-research-analyst` with this prompt:
...
2. **Learnings search** — dispatch `systematic:learnings-researcher` with a brief summary of the ideation focus.

3. **Issue intelligence** (conditional) — if issue-tracker intent was detected in Phase 0.2, dispatch `systematic:issue-intelligence-analyst` with the focus hint. If a focus hint is present, pass it so the agent can weight its clustering toward that area. Run this in parallel with agents 1 and 2.
...
- **Tools available + user asked**: Dispatch `systematic:slack-researcher` with the focus hint in parallel with other Phase 1 agents. Include findings in the grounding summary.
