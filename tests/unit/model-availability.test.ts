...
describe('getAvailableModels', () => {
  ...
  describe('edge case: cache path resolution', () => {
    ...
    test('falls back to $HOME/.cache/opencode/models.json when XDG_CACHE_HOME is unset', async () => {
      ...
      expect(result.has('google/gemini-2-flash')).toBe(true)
    })
  })

  describe('edge case: OPENCODE_MODELS_URL', () => {
    test('prefers models-*.json over models.json when OPENCODE_MODELS_URL is set', async () => {
      const cacheDir = path.join(testDir, 'cache', 'opencode')
      fs.mkdirSync(cacheDir, { recursive: true })
      ...
      // Write a models-<hash>.json with anthropic (should be used)
      fs.writeFileSync(
        path.join(cacheDir, 'models-abc123.json'),
        JSON.stringify({ anthropic: { models: { 'claude-opus-4-7': {} } } }),
      )
    ...
    test('falls back to models.json when OPENCODE_MODELS_URL is set but no models-*.json exists', async () => {
      const cacheDir = path.join(testDir, 'cache', 'opencode')
      fs.mkdirSync(cacheDir, { recursive: true })
  ...
  describe('error path: wrapped future cache format', () => {
    test('treats { providers: { ... } } as schema-mismatched and returns empty Set', async () => {
      const cacheDir = path.join(testDir, 'cache', 'opencode')
      fs.mkdirSync(cacheDir, { recursive: true })
      fs.writeFileSync(
        path.join(cacheDir, 'models.json'),
