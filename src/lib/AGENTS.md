...
| `routing-resolver.ts` | `resolveRouting`, `qualifierResolvesWithoutModel`, `collectLegacyPiSubagentsThinkingWarnings`, `RoutingTarget`, `RoutingResolution` | Pure per-target/harness routing precedence (agent block > agent flat > category block > category flat) for `model` and the opencode `variant` / pi `thinking` qualifier, with legacy `pi_subagents.thinking` fallback; backs the loader's post-merge qualifier-requires-model check |
