...
| `review-pipeline.ts` | `screenReviewReturn`, `ScreenReviewReturnInput` | Pure, side-effect-free `screen` phase implementation: parses one raw reviewer return (a JSON string or already-decoded value), reuses `validateReviewReturnValue` from `review-return-validator.ts` for the `SubAgentReturnSchema` structural check, and binds the parsed reviewer identity to the parent-expected persona. Rejection is whole-payload only (malformed JSON, schema failure, or identity mismatch); findings that pass structural validation are admitted with a stable `<reviewer>#<original-index>` input ID derived from their position in the original payload. Diagnostics carry only the persona, JSON path, and one fixed reason (`schema validation`, `malformed JSON`) -- never payload content. Admission never depends on the process environment; never reads `process.env`, the filesystem, or the clock |
