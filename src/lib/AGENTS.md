...
| `config-schema.ts` | `SystematicConfigSchema`, `SystematicConfigParsed`, `validateConfig`, `SECURITY_OVERLAY_FIELDS`, `AgentOverlaySchema`, `CategoryOverlaySchema`, `BootstrapSchema` | Canonical Zod schema for user config; security field list |
...
| `review-return-validator.ts` | `runReviewReturnValidator`, `validateReviewReturnValue`, `VALIDATE_REVIEW_RETURN_USAGE`, `MAX_REVIEW_RETURN_BYTES`, `MAX_PROJECTED_ISSUE_LINES`, `formatReviewReturnValidationFailure`, `ReviewReturnValidatorOptions`, `ReviewReturnValidation`, `ReviewReturnIssue`, `ReadChunk` | Bounded, no-write structural validator for one raw `ce:review` persona return read synchronously from stdin (1 MiB byte cap, stops at cap + 1); backs `systematic validate-review-return`. Accepts only `SubAgentReturnSchema`; exit 0 valid, 1 empty/oversized/invalid-UTF-8/malformed/schema-invalid, 2 usage/TTY/read failure. Injectable `argv`/fd/TTY/chunk-reader/sinks keep it unit-testable without global mutation, and diagnostics project only safe Zod paths plus issue codes (never messages, key names, or values) |
| `review-pipeline-contract.ts` | `ScreenInputSchema`, `ScreenOutputSchema`, `ScreenRejectedSummarySchema`, `PrepareInputSchema`, `PrepareOutputSchema`, `AGGREGATE_STDIN_BYTE_CAP`, `PER_FINDING_BYTE_ASSUMPTION`, `AGGREGATE_BYTE_CAP_HEADROOM`, `ROUTE_REFUSAL_TABLE`, `isRouteTransitionAllowed`, `PipelineRoute` | Strict `review-pipeline.v1` Zod contracts for the `ce:review` synthesis pipeline's `screen` and `prepare` phase envelopes, composed from `SubAgentReturnSchema`/`ParentFindingSchema` in `review-artifact-schema.ts` and its exported `MAX_FINDINGS`/`MAX_PERSONAS`/`MAX_INPUT_ID_LENGTH`/`MAX_REASON_LENGTH` bounds rather than restating them. `AGGREGATE_STDIN_BYTE_CAP` derives the `prepare` stdin bound from those same maxima. `ROUTE_REFUSAL_TABLE` and `isRouteTransitionAllowed` encode narrowing-only `autofix_class`/`owner`/`requires_verification` transitions as authored data for later merge/finalize phases to consume. `PrepareOutputSchema.surviving_findings` carries every confidence-gate survivor's own finding fields (composed from `ParentFindingSchema`) plus its stable input ID and submitting reviewer, so the merge phase can derive a merged finding's severity/`pre_existing`/route and submitters from real contributing-input data instead of nothing; suppressed findings never appear there |
| `review-pipeline.ts` | `screenReviewReturn`, `ScreenReviewReturnInput`, `prepareReviewCandidates`, `PrepareReviewCandidatesInput`, `PrepareReviewCandidatesResult`, `PrepareRejection`, `PrepareOutput`, `normalizeRepoRelativePath`, `validateAdjudication`, `ValidatedMergedGroup`, `ValidatedDeclinedSingleton`, `ValidatedAdjudication`, `AdjudicationRejection`, `ValidateAdjudicationResult`, `deriveMergedFindingFields`, `MergeContributingFindings`, `MergedFindingModelDecision`, `DeriveMergedFindingInput`, `DerivedMergedFindingFields`, `MergedFindingRejection`, `DeriveMergedFindingResult`, `applyReviewAdjudication`, `ApplyReviewAdjudicationInput`, `ApplyReviewAdjudicationResult`, `MergeOutput`, `reconcileValidatorResults`, `ReconcileValidatorResultsInput`, `ReconcileValidatorResultsOutput`, `ReconcileValidatorResultsResult`, `ReconciledFinding`, `ValidatorLifecycleFailure`, `finalizeReviewDispositions`, `FinalizeReviewDispositionsInput`, `FinalizeReviewDispositionsOutput`, `FinalInputDisposition`, `FinalizedInputDisposition`, `RejectedPayloadWeight`, `FinalDispositionCounts`, `FindingActionRoute`, `QueuedFinding`, `PreExistingFinding` | Pure, side-effect-free `screen`, `prepare`, merge-adjudication-validation, single-merged-finding-derivation, and full-merge-phase-assembly implementations. `applyReviewAdjudication` is the top-level merge phase: it validates the adjudication partition via `validateAdjudication`, derives every merged group's and every declined/passthrough singleton's fields via `deriveMergedFindingFields` (a rejection from either aborts with no partial output), computes the purely mechanical validator request set (every finding that is `P0`/`P1` or `requires_verification: true`), sorts by severity then confidence descending then normalized path then line then fingerprint, and parses the assembled result through `MergeOutputSchema`. `screenReviewReturn` parses one raw reviewer return (a JSON string or already-decoded value), reuses `validateReviewReturnValue` from `review-return-validator.ts` for the `SubAgentReturnSchema` structural check, and binds the parsed reviewer identity to the parent-expected persona; rejection is whole-payload only (malformed JSON, schema failure, or identity mismatch). `prepareReviewCandidates` combines every selected persona's screen result plus its dispatch metadata, rejects an oversized aggregate before parsing it (`AGGREGATE_STDIN_BYTE_CAP`), cross-references duplicate personas/input IDs and selected-dispatch/screen-result correspondence, applies the confidence gate (surviving at >= 0.60, or a `P0` finding at >= 0.50) before any grouping, and groups surviving findings by `normalizeRepoRelativePath` into candidate groups only when at least two distinct personas share a file -- it never merges, and each wire-level candidate group member now carries its own stable input ID and line (no per-member file, which is always the group's file) so downstream consumers can verify member ordering and a merge decision's representative line against real data, and also emits every survivor's own finding fields plus reviewer in `PrepareOutput.surviving_findings`, sorted by stable input ID, so later phases can derive from real contributing-input data rather than nothing. `validateAdjudication` checks that a model's merge/decline decisions form a valid partition over the real `PrepareOutput` envelope. `deriveMergedFindingFields` takes one merge group's contributing survivors (a non-empty tuple, since `CandidateGroupSchema.members` is `.min(2)`) plus the model's decision fields for that group and derives severity (highest), submitters (distinct reviewers, sorted), confidence (highest, +0.10 capped at 1.0 when submitters plus validated agreement credit span >= 2 distinct reviewers), `pre_existing` (true only if every contributor is), a fingerprint (from `normalizeRepoRelativePath`, the representative line, and severity), and route (the meet over `ROUTE_REFUSAL_TABLE` per field, narrowed further only when the model supplies both a proposed route and a reason, refusing any widening via `isRouteTransitionAllowed`); agreement credit is rejected when duplicated, already a submitter, or naming a reviewer absent from the supplied returned-reviewers set's eligible candidate-group members (each cited exactly once, no unknown/suppressed/cross-group ID, a merged decision's representative `line` among its group members' lines), rejecting the whole envelope with a fixed reason code and safe JSON path on any violation rather than repairing it; it computes no merged-finding fields (severity/confidence/provenance/routing), leaving that to a later derivation step. Diagnostics across all three phases carry only a fixed reason code and a safe JSON path -- never payload content or exception text. None of the three phases read `process.env`, the filesystem, or the clock. `reconcileValidatorResults` is the finalize-phase validator-lifecycle-reconciliation step: it classifies every merged finding as validated/filtered/uncertain against its `ValidatorLifecycleResultSchema` outcome, rejecting whole-payload on a missing, duplicate, or unrequested result. `finalizeReviewDispositions` derives every admitted input's final disposition (`suppressed`/`filtered`/`merged`/`surviving`, sourced from `confidence_dispositions` and the reconciled `filtered_input_ids` -- never from anything a model supplied), the weighted `FinalDispositionCounts` (a rejected payload contributes one weighted `rejected_finding_count`, not one row per finding), the pre-existing/new finding split (mirroring `derivePreExisting`'s all-contributing-inputs rule against `surviving_findings`), and the three mutually exclusive, collectively exhaustive action queues (`fixer`/`residual`/`report_only`) partitioned by each finding's mechanically-derived `owner`; a filtered finding enters no queue, and a finding left uncertain by a failed/unavailable validator stays actionable but is marked `unconfirmed` |
| `agent-colors.ts` | `isValidAgentColor`, `OPENCODE_AGENT_COLOR_TOKENS` | Color validator (hex or named token) + accepted token enum |
| `config-handler.ts` | `createConfigHandler`, `ConfigHandlerDeps`, `formatAgentDescription`, `toTitleCase`, `readEmittedAgentPermission`, `EmittedAgentPermission` | OpenCode config hook (collects + emits all assets) |
