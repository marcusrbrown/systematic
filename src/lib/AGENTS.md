...
| `review-pipeline.ts` | `screenReviewReturn`, `ScreenReviewReturnInput` | Pure, side-effect-free `screen` phase implementation: parses one raw reviewer return (a JSON string or already-decoded value), reuses `validateReviewReturnValue` from `review-return-validator.ts` for the `SubAgentReturnSchema` structural check, binds the parsed reviewer identity to the parent-expected persona, and recursively screens every string leaf against a caller-supplied environment snapshot (structural `$NAME`/`${NAME}`/`process.env.NAME`/`os.environ[...]`/`NAME=value` detectors plus bounded embedded-value matching). An offending leaf inside one finding drops only that finding (stable `<reviewer>#<original-index>` input IDs so a dropped sibling never renumbers the rest); an offending leaf outside any finding rejects the whole payload. Diagnostics carry only the persona, JSON path, and one fixed reason (`schema validation`, `environment-value detection`, `malformed JSON`) -- never the matched value or variable name. Never reads `process.env`, the filesystem, or the clock |
