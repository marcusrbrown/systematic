...
A project-defined profile name can never shadow one you or `$OPENCODE_CONFIG_DIR` config already defined: bundle lookup checks custom, then user, then project **last**. If your `personal` profile and a repository's `personal` profile both exist, yours always wins the name.
