...
Only `profile` (the selector) is project-settable. `profiles` (the map of bundle definitions) is trust-protected exactly like `model`, `variant`, `permission`, and `skills` — a project may choose among the profiles you've defined, but it cannot define new ones or see into ones you haven't shared. A `profiles` map set in project config is ignored with a warning.
...
If the selected name doesn't match a defined profile, Systematic warns once and falls back to your own `profile` default (if you have one and it resolves); if that's also undefined, it falls back to base configuration with no profile active. Run `systematic config show` to see the active profile, which source selected it, and any fallback that occurred.
