...
A well-structured plugin works immediately. Systematic uses the `config` hook to auto-register all bundled skills, agents, and commands during the OpenCode startup sequence.
...
- **Commands**: Workflow commands use the `workflows:` prefix, while utility commands use `systematic:`.

## Generate Docs from Source

Documentation should never be out of sync with the code. Systematic treats Markdown assets (skills, agents, commands) as the source of truth for its documentation site.
