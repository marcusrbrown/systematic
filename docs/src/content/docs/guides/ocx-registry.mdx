...
description: Install individual Systematic skills and agents via the OCX registry — pick only what your project needs, or install everything at once.
sidebar:
  order: 6
---

[OCX](https://github.com/kdcokenny/ocx) is a package manager and component registry for AI agents and developer tools. It allows you to install Systematic components—skills and agents—individually or as themed bundles, without needing the full npm plugin.
...
ocx add systematic/agent-security-reviewer
```

### Skill Slash Commands
Install skills that remain invocable as slash commands in OpenCode:

```bash
# Add the document review skill
ocx add systematic/document-review

# Add the orchestration skill
ocx add systematic/orchestrating-subagents
```
...
```bash
# Install all bundled Systematic skills
ocx add systematic/skills

# Install all bundled Systematic agents
...
### OMO Profile
The `omo` profile harmonizes Systematic with [Oh My OpenAgent (OMO)](https://github.com/code-yeongyu/oh-my-openagent). It configures both plugins to work together, disabling redundant OMO features in favor of Systematic's structured workflows.
...
```bash
ocx diff systematic/frontend-design
```
...
```bash
ocx update systematic/frontend-design
```
...
```bash
ocx update systematic/frontend-design@1.2.0
```
