...
Both forms exit `1` when the configuration fails to load, printing the file and key that rejected it, and `0` otherwise — so a script can branch on the exit status rather than parsing the output.
