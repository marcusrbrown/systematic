...
- [x] **Goal:** Make Systematic improve automatically as users adopt better models instead of pinning tasks to the model market of August 2026.
- **Completed:** 2026-08-17. `src/lib/source-model-defaults.ts` and `src/lib/model-availability.ts` are deleted rather than narrowed; no provider-name routing survives in `src/` (v3.10.0). Every bundled agent reference resolves to a real agent and is covered by `checkReferenceIntegrity`, and the dead `model:` task-tool parameter instruction is gone (v3.10.2). The remaining provider literals are JSON Schema `examples` documenting the `provider/model` shape and sample code inside `skills/agent-native-architecture/references/`, neither of which is routing policy. Residual work — a gate that rejects non-canonical agent identifiers before they rot — is reference integrity rather than model routing and is tracked in issue #800.
...
- **Approach:**
  ...
  - Close the unvalidated-dispatch-identifier surface, because a skill that names an unresolvable agent silently bypasses the user's model policy. The qualified `systematic:<category>:<name>` form is canonical: `checkReferenceIntegrity` validates it against real agent files (ARCHITECTURE invariant #4), and the Claude Code build translates it in generated output only. Never convert it to bare names — `docs/solutions/best-practices/qualified-persona-ids-are-canonical-validated-references-2026-07-17.md` records that refactor collapsing mid-implementation. The gap was the inverse: identifiers in any other spelling are invisible to the gate and rot silently. Shipped in v3.10.2 — 24 phantom `ce-`-prefixed references, a nonexistent `subagent_type`, three unbound prose roles, and the dead `model:` parameter instruction (issue #784) are all resolved. Per-harness dispatch projection remains I6's work.
- **Test scenarios:**
  ...
  - Workflow dispatch: every agent identifier a bundled skill names resolves to a real bundled agent and is covered by a validation gate, so a typo or stale convention fails CI instead of shipping.
- **Child-plan entry gate:** inventory each current default's consumer and fallback behavior, select one deletion wedge, and define config precedence and rollback without introducing scorecard-driven routing. For the dispatch-identifier wedge, additionally inventory every agent reference in bundled content and classify each as canonical-qualified, unvalidated-other-form, unbound prose, or phantom — and confirm against the 2026-07-17 prior art before proposing any change to the qualified form.
