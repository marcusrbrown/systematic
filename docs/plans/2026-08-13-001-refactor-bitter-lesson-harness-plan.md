...
- **Approach:**
  ...
  - Close the unvalidated-dispatch-identifier surface. The qualified `systematic:<category>:<name>` form is canonical and correct: `checkReferenceIntegrity` validates it against real agent files (ARCHITECTURE invariant #4), and the Claude Code build translates it to `systematic:<name>` in generated output only. Do not convert it to bare names — see `docs/solutions/best-practices/qualified-persona-ids-are-canonical-validated-references-2026-07-17.md`, where that exact refactor collapsed mid-implementation because stripping the qualified form removes validation coverage. The real gap is the inverse: identifiers written in any other form get no validation at all, and rot silently. Concrete instances — `skills/ce-plan/references/deepening-workflow.md` carries ~22 `ce-`-prefixed references (`ce-spec-flow-analyzer`, `ce-deployment-verification-agent`) matching no bundled agent and no registered dispatch key, while `skills/deepen-plan/SKILL.md` holds the same roster in canonical form; `skills/agent-native-audit/SKILL.md:37` names `subagent_type: Explore`, which exists nowhere; `ce:compound` Phase 1 names three roles in prose with no identifier, inviting free substitution that silently changes model tier; and `ce:review`/`ce:ideate` instruct a `model:` parameter OpenCode's `task` tool does not accept, so per-agent model policy is bypassed. Content-integrity reports clean with all of it present. See issue #784 for the `model:`-parameter mode. Per-harness dispatch projection is I6's work — this wedge only makes every named identifier real and validated.
- **Test scenarios:**
