...
| R14 (one-time, unforgeable attestation requiring host-recognized interactive user-role provenance, stable host identity, and one-use run/resource/transition binding) | Phase A compares only allowlisted hook input/output and persisted `UserMessage` field projections/hashes/booleans across unauthenticated external SDK prompt, Basic-auth external SDK prompt (`OPENCODE_SERVER_PASSWORD`, optional username), and assistant-triggered/plugin-client-created user message; Phase B is conditional on discovering a real caller identity/binding field | U2 |
...
| R17 (static plugin config reaches the plugin, and disabled/unavailable values can be represented without claiming protection) | Status transport probe observes static plugin config delivery and transport representation of disabled/unavailable values; default-on/global-vs-session authorization is later guard testing, and typed interactive user-role provenance remains U2 | U1/U2 |
...
- The user-approved disablement-authority decision remains a future implementation constraint: static operator configuration alone may disable protection globally; only a host-recognized interactive user, via typed confirmation, may disable protection for the current session; assistant text, tool arguments, and unconfirmed session settings never authorize either scope. U1 does not claim to prove this policy before the guard exists. U1 probes only static plugin-config delivery and whether status transport can represent disabled/unavailable values; typed interactive user-role provenance remains U2.
...
| Phase A host-authenticated caller-field prerequisite, followed by conditional one-use attestation testing | R14 | U2 | Compare only allowlisted hook input/output and persisted `UserMessage` projections/hashes/booleans for unauthenticated external SDK, Basic-auth external SDK, and assistant/plugin-created messages. If no stable, unforgeable plugin-visible principal field exists, R14 is immediately `unsupported`; no replay/consumption/cross-resource/fork scenarios run without that field and an unbuilt guard |
...
Each remaining question is resolved only by a classified probe outcome, not by further reading or design discussion:
...
- R14 caller-identity question — resolved identically on exact hosts `1.18.3` and `1.18.4`: Phase A is `unsupported`, `phaseBRequired:false`; all three message origins were healthy but indistinguishable, Basic auth added transport authentication without principal provenance, and no host-backed identity substrate justified Phase B policy tests.
...
flowchart LR
  ...
  subgraph U2["U2: Lifecycle, correlation, attestation"]
    ...
    T["R14 Phase A caller-field prerequisite"]
...
Goal: Determine, hermetically, whether OpenCode's restart/compaction/fork primitives preserve an exact synthetic receipt-bearing ToolPart; whether the `subagent-stop-probe.ts` correlation seam provides two-channel host-native parent-child lineage or requires an actually-exercised Systematic-controlled fallback; and, in Phase A only, whether a real host-authenticated plugin-visible caller identity/binding field exists for R14. Phase B attestation rejection testing is conditional on that prerequisite and an actual guard. This unit does not build the fallback or guard for production use or prove either is production-ready — any demonstration proves a substrate, not final production equivalence.
...
- The shared fixture needs one minimal backward-compatible auth option for R14 Phase A: run an unauthenticated external SDK prompt, a Basic-auth external SDK prompt using `OPENCODE_SERVER_PASSWORD` and an optional username, and an assistant-triggered/plugin-client-created user message. The manifest records only `authEnabled:boolean`, never credentials or headers. Compare only allowlisted hook input/output and persisted `UserMessage` field projections, hashes, and booleans. In v1, `prompt`/`promptAsync` has no top-level `synthetic`; text-part `synthetic` is forgeable/cooperative and cannot serve as attestation identity. Basic auth authenticates server access but no principal, username, or client identity reaches `chat.message` or persisted `UserMessage`.
- R14 is Phase A only unless it discovers a real field. If authenticated, unauthenticated, and assistant/plugin-created messages expose no stable, unforgeable, plugin-visible principal field distinguishing genuine interactive input from programmatic or assistant-reachable prompt creation, classify R14 `unsupported` immediately. Do not run replay/consumption/cross-resource/fork policy scenarios against an unbuilt guard. Only if Phase A finds such a field may a later Phase B test one-use/replay/substitution behavior, with no invented token/guard; `synthetic` is not assumed to be a security boundary.
...
- Attestation Phase A path: compare only allowlisted hook input/output and persisted `UserMessage` field projections, hashes, and booleans for unauthenticated external SDK prompt, Basic-auth external SDK prompt (`OPENCODE_SERVER_PASSWORD`, optional username), and assistant-triggered/plugin-client-created user message. v1 `prompt`/`promptAsync` has no top-level `synthetic`; text-part `synthetic` is forgeable/cooperative, and Basic auth adds no principal/username/client identity to `chat.message` or persisted `UserMessage`. If authenticated, unauthenticated, and assistant/plugin-created messages expose no stable, unforgeable plugin-visible principal field, R14 is `unsupported` and no Phase B replay/consumption/cross-resource/fork scenarios run. If such a field exists, Phase B may test one-use/replay/substitution behavior without an invented token or guard.
...
Verification: U2 is complete for R4, R13, and R14 on both mandatory exact-host cells. R4 is resolved per the lifecycle record below; `primary-supported` is not claimed because compaction is unavailable, while restart and fork remain substrate evidence only. R13 is `primary-supported` only for the tested foreground path. R14 is Phase A `unsupported` with `phaseBRequired:false`, so replay/consumption/substitution/cross-resource/fork policy tests remain not-run. U2 completion does not mark the requirements satisfied; R3/R22 remain deferred, U3 is complete, and U4 is complete as an evidence synthesis only. No result claims a production guard, ledger, or invented attestation token.
...
Probe sources: `tests/manual/receipt-lineage-probe.ts` and `tests/manual/receipt-attestation-probe.ts`. The existing vendored OpenCode source references in this plan provide source context; the classifications below are exact-host runtime evidence and contain no raw hashes, JSON, or internal task/session metadata.

| Exact host | R13 foreground lineage | R14 Phase A caller identity |
|---|---|---|
| `1.18.3` | `primary-supported` | `unsupported` |
| `1.18.4` | `primary-supported` | `unsupported` |
...
- R14 evidence was identical on both hosts: Phase A classified `unsupported` with `phaseBRequired:false`; unauthenticated external SDK, Basic-authenticated external SDK, and plugin-created sibling-session prompts were all healthy. Basic auth transport was accepted but surfaced no principal provenance. Hook and persisted projections were indistinguishable across all three origins; `authSpecificPrincipalFieldCount:0`; candidate fields `user`, `username`, `principal`, `clientID`/`clientId`, `actor`, `identity`, and `auth` were all absent. Part-level `synthetic` was absent and is not identity evidence regardless. Replay/consumption/substitution/cross-resource/fork policy tests correctly remained not-run because Phase A found no host-backed substrate. There was no version divergence.
...
- Negative consequence: R4's compaction primary path is unavailable on both exact hosts; fresh-readback fallback remains the only applicable design path, but that fallback was not itself proven. Negative consequence: R14's caller-provenance prerequisite is absent, so replay, reuse, and substitution policy tests were correctly not run.
- Stop/not-go evidence finding: the evidence does not support treating the original requirement set as ready for implementation as written. This finding does not authorize, narrow, rewrite, or choose next steps.
...
Verification: U4 is complete because the synthesis is complete, not because the requirements are satisfied. `## Capability Findings` contains one classified outcome for every probed runtime group, R3/R22 are explicitly deferred/incomplete, and R20 is documentation-only/excluded. The synthesis records sufficient substrate evidence individually, identifies R4 and R14 as unsupported and therefore makes the probed aggregate insufficient, and preserves the no-authorization boundary.
...
| R14 | U2 | `unsupported` | Insufficient: no host-backed direct-user provenance across the three origins on either version. Phase B, including replay/reuse/substitution policy tests, was correctly not run. |
...
| The R14 attestation channel lacks a field sufficient to distinguish genuine interactive input from programmatic or assistant-reachable prompt creation. | U2 compares the three required message origins and auth modes using only allowlisted hook input/output and persisted `UserMessage` projections/hashes/booleans; absence of a stable, unforgeable plugin-visible principal field is an immediate `unsupported` result, and replay/consumption/cross-resource/fork scenarios are not run. Phase B is conditional on discovering such a field and an actual guard. |
