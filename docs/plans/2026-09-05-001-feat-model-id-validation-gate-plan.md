...
- `docs/src/content/docs/guides/pi-subagents.mdx:86-101` — a structural fenced JSONC example carrying three `"model"` values (`:91`, `:94`, `:99`); `:121` carries two inline `key: value` spans, `` `model: github-copilot/gpt-5.6-luna` `` and `` `model: openai/gpt-6-astra` ``, grounding the structural/inline extraction split used throughout Unit 2.
...
- **`generate-config-schema.ts` gains an object-returning JSON Schema producer, factored out of `generateSchemaContentFromSchema`**: the TS-surface extractor needs the same `z.toJSONSchema(schema, { target: 'draft-7', reused: 'ref' })` call the full generator already makes, but not its `$id` stamping or its `formatJsonWithBiome` formatting pass. Sharing the object-returning step in memory, rather than re-deriving it, means the gate can never disagree with the published schema about what a `.meta()` call contains, and never needs to spawn Biome or supply a version argument it has no use for. The extractor discriminates candidates by **position** — reached as a property named `model`, or nested under one, inside an `examples` value — not by the id-shape regex alone: run directly against the emitted schema, shape-only matching (any `examples` string matching the id pattern, with no positional constraint) returns 7 distinct strings but only 3 are real ids; the rest are a schema `$id` URL, a qualified agent id, and two file paths (full breakdown in Unit 2 Approach).
...
**Approach:**
- **Two verification passes before the check goes live.** First, run the exact object-returning JSON Schema walk (the same `z.toJSONSchema(schema, { target: 'draft-7', reused: 'ref' })` call, plus a recursive walk over `examples`) directly against `src/lib/config-schema.ts`'s live schema before writing the extractor's own tests, confirming the position-based discriminator's real/false-positive split described below still holds. Then implement the three extractors and `checkModelIdentifiers` against `tests/fixtures/model-ids/`, proven by unit tests, before touching the live repository. Only after those tests pass, re-run the same schema walk plus the JSONC/Markdown extractors against the real tree with Unit 1's freshly generated snapshot. Any finding at that point is either a real stale id — fixed in this same unit and named in the PR body — or an extractor false positive — fixed in the extractor, never suppressed. Only then wire `checkModelIdentifiers` into `checkContentIntegrity`'s returned `CheckResult` and `main`'s violation total, so the check goes live against a verified-clean baseline.
- **TS surface** (`src/lib/config-schema.ts`): `scripts/lib/model-id-extractors.ts` obtains the JSON Schema object via the object-returning export factored out of `scripts/generate-config-schema.ts`'s `generateSchemaContentFromSchema` — it never parses TypeScript source. With `target: 'draft-7'`, the dedup key in the emitted schema is `definitions`, not `$defs`; `reused: 'ref'` collapses a shared shape like `modelSchema` into one `definitions` entry referenced from every `model` property via `$ref`.
- **Position, not shape, is the discriminator.** A string is a **structural** candidate only when it is (i) the value of a `model` key inside an `examples` value, or (ii) an `examples` entry on the schema node reached as a property named `model` — resolved through `definitions`, `$ref`, and every combinator (`allOf`, `anyOf`, `oneOf`) — the draft-7 metadata wrapper is `{ allOf: [{ $ref }], description, examples }`, so the hop from the `model` property to the node carrying `examples` goes through `allOf` first, and the nullable wrapper carries `examples`/`description` on its `anyOf` node while the id-shape `pattern` sits one `$ref` hop further in, on the branch `anyOf[0]` points to. `pattern` cannot be the discriminator: the keyword also serves unrelated fields elsewhere in the schema (a bare non-whitespace check, a hex-color check), and it never sits on the same node as the `model` property's own `examples`. Matching by shape alone was probed directly against the emitted schema and produces 7 distinct values, only 3 of them real ids (`anthropic/claude-sonnet-5`, `anthropic/claude-opus-5`, `openai/gpt-6-astra`) — the other 4 are a schema `$id` URL, the qualified agent id `review/security-reviewer` (`config-schema.ts:642`), and two file paths (`.opencode/custom-prompt.md`, `~/.config/opencode/bootstrap.md`) — confirming the position-based rule, not the shape regex, is what the extractor keys on. Accepted residual: a model id placed in an example under a key other than `model` is invisible to the TS extractor; that is the price of excluding the qualified agent id, and no shape refinement could pay it otherwise.
- Every backtick-delimited span inside a `description` string is an **inline** candidate (documentation prose, not a config value). Twelve of the thirteen model-id-shaped sites in `config-schema.ts`'s source are nested inside an object rather than a flat array entry — background motivation for why the walk descends recursively, not what the extractor's tests assert against (they assert against the emitted schema's shape, not the source-site count).
...
**Verified, not assumed:** the TS surface's position-based discriminator was verified directly against the emitted schema (`z.toJSONSchema(SystematicConfigSchema, { target: 'draft-7', reused: 'ref' })`), not assumed from the source shape: shape-only matching returns 7 distinct `examples` strings, and only the 3 reached through a `model`-named property are real ids (see Key Technical Decisions for the full split). Grepping `docs/src/content/docs/` and `skills/` for `provider/model`-shaped text found it mostly in bare prose (`configuration.mdx`'s generated `.meta()` description text, "Model identifier in provider/model format..."), outside any code span or fence and so not a false-positive risk under this unit's extraction rule. `writing-skills/references/foundation-conventions.md:123` does carry inline code spans `anthropic/claude-...` and `openai/gpt-...` — elision placeholders, covered by the elision-placeholder allowlist rule above rather than by extraction exclusion. No other placeholder-shaped id appears in a code span or fence today; both sanctioned forms are allowlisted so future documentation can use either without failing the gate.
...
- Happy path: extraction through the `allOf` → `$ref` → `anyOf` chain reaches the `modelSchema` definition's own `examples: ['anthropic/claude-sonnet-5', null]` entry, and rule (ii) contributes a non-empty result on the live schema (without `allOf` traversal it contributes nothing).
- Edge case: a model id placed in an `examples` value under a key other than `model` is not extracted, pinning the accepted residual as intentional.
- Edge case: a non-string `examples` entry (`null`, a number, a boolean) is skipped without erroring.
- Edge case: non-model `examples` strings that happen to match the id shape — a schema `$id` URL, `.opencode/custom-prompt.md`, `~/.config/opencode/bootstrap.md` — are not extracted, since none is reached through a `model`-named property.
- Edge case: the qualified agent id `review/security-reviewer` (`config-schema.ts:642`) is not extracted, pinning the discriminator to position rather than shape.
- Integration: an example nested inside a schema's `definitions` entry (produced by `reused: 'ref'` deduplication) is still reached by the recursive walk.
- Integration: the extractor's tests assert against the emitted schema's shape — three distinct real ids across every `model`-reached `examples` value — not the thirteen source-level `.meta()` sites in `config-schema.ts`; the source-site count (12 of 13 nested) is background rationale for the recursive walk, not the assertion target.
...
**Verification:**
- The object-returning JSON Schema walk, run directly against the live `src/lib/config-schema.ts` schema, reproduces the position-based real/false-positive split recorded in Key Technical Decisions before `checkModelIdentifiers` is wired into `checkContentIntegrity`.
...
| Markdown/MDX extraction under- or over-matches in content planning didn't anticipate | The gate is hard-fail (R6, exact match only), so an under-match is a silent gap caught only by review, not by the gate itself (on the TS surface the accepted under-match is an id under a non-`model` key); over-matching is bounded by the code-span/fence-only rule plus the context-dependent provider filter (applied only to inline candidates, never to structural values, per Key Technical Decisions). |
