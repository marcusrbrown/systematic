...
- **TS surface** (`src/lib/config-schema.ts`): `scripts/lib/model-id-extractors.ts` imports `SystematicConfigSchema` and derives its JSON Schema the same way `scripts/generate-config-schema.ts`'s `generateSchemaContentFromSchema` does, then walks that JSON Schema object in memory — it never parses TypeScript source. Every `examples` array entry is a **structural** candidate; every backtick-delimited span inside a `description` string is an **inline** candidate (documentation prose, not a config value). Reusing the schema's own production path means the gate and the published JSON Schema can never disagree about what a `.meta()` call contains.
...
- **Context-dependent resolution, not a blanket filter.** Every structural candidate is validated against the snapshot unconditionally: a provider segment absent from the snapshot's derived provider set fails as `model-id-unknown-provider`; a known provider with no matching model id fails as `model-id-unknown`. Inline candidates are filtered first — discard any token containing `://`, starting with `@`, or ending in a file extension, then discard any whose provider segment is not in the snapshot's provider set (paths, package names, and similarly shaped non-ids) — only a surviving inline candidate is checked, failing as `model-id-unknown` if absent from the snapshot. The literal string `provider/model` is allowlisted everywhere, structural or inline, as the one sanctioned schematic placeholder; no other placeholder-shaped string is exempt.
- `MODEL_ID_EXCLUDED_PATHS` is a single array constant of repo-relative paths/globs: `docs/src/content/docs/guides/model-defaults-migration.mdx`, `docs/src/content/docs/guides/with-without-systematic.mdx`, `docs/plans/**`, `docs/solutions/**`, `tests/**`, `evals/**`, `tests/manual/**`. The check fails (`model-id-stale-exclusion`) if any listed literal path no longer exists on disk — mirroring the existing exclusion-hygiene pattern already used elsewhere in this file (e.g., `HOOK_PARITY_EXEMPTIONS`).
- The gate reads only the committed snapshot file from disk; it makes no network call (R13).

**Verified, not assumed:** grepping `docs/src/content/docs/` and `skills/` for `provider/model`-shaped text found it only in bare prose (`configuration.mdx`'s generated `.meta()` description text, "Model identifier in provider/model format...") and in `writing-skills/references/foundation-conventions.md`'s prose about `model: inherit` history — neither is inside a code span or fenced block, so neither is a false-positive risk under this unit's extraction rule. No placeholder-shaped id appears in any code span or fence today either; the one sanctioned placeholder (`provider/model`) is allowlisted for future use, not because it's needed against current content.
...
- `scripts/generate-config-schema.ts:361-414` (`generateSchemaContentFromSchema`) — the JSON Schema production path the TS-surface extractor reuses in memory instead of parsing `config-schema.ts` as text.
...
| False positives from placeholder ids in code fences | Verified by direct grep of `docs/src/content/docs/` and `skills/`: no placeholder-shaped id appears in a code span or fence today. The gate still allowlists exactly one sanctioned placeholder, the literal `provider/model`, for future schematic examples; any other placeholder-shaped string must resolve to a real id or be excluded. |
