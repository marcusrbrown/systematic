...
- `.github/workflows/main.yaml` — job conventions (Bun + Node 24 setup, `bun install --frozen-lockfile`, the `release` job's `needs` list). No job in any workflow sets `timeout-minutes` today; `host-contract` is the first, and the `docs` job's `actions/cache` step is a Playwright browser cache, not a Bun one.
...
- **One probe, two outcomes.** A shared helper runs `bunx opencode-ai@<pin> --version` under a caller-supplied isolated env with a bounded timeout and returns a classification; it is a plain per-call function and never throws. The fixture exports a memoizing `isOpencodeAvailable()` function rather than an eagerly computed constant: the probe runs the first time an integration module evaluates `describe.skipIf(!isOpencodeAvailable())`, and never when a unit test imports the fixture for its pure helpers or the pin re-export (`tests/unit/receipt-workflow-host.test.ts` and `tests/unit/eval-contract.test.ts` both do), so `bun test tests/unit` and the required `test` job never spawn the launcher. `eval-runner.test.ts` memoizes its own gate the same way. The eval runner calls the probe per case with its own timeout, so nothing memoizes across cases there. Success means the reported version equals the pin. Anywhere else, the suites skip with a named reason that includes the status and captured stderr. When `SYSTEMATIC_REQUIRE_OPENCODE=1` (set only by the CI job), the test modules pass that classification to a second exported function that throws when it is not `available`, so the file fails to load and a green CI run can never be a run of zero tests. The eval runner's per-case path never calls the throwing function. Rationale: `availability-guards-must-check-executability` plus the flow analysis's silent-skip finding.
...
**Files:**
- Modify: `tests/integration/fixtures/receipt-workflow-host.ts` (`OPENCODE_AVAILABLE`, `startOpencodeServer`, `startExactOpencodeServer`, `prewarmExactOpencode`, `getExactNpmCacheDir`, `runOpencode`, and `buildChildEnv` for the `BUN_INSTALL_CACHE_DIR` override), `scripts/eval-cases/opencode.ts` (`startOpencodeHost`), `scripts/run-evals.ts` (`verifyExactOpencodeRuntime`)
...
**Approach:**
- Every spawn of an OpenCode host or command becomes `bunx` with `opencode-ai@<pin>` as the first argument, followed by the existing OpenCode arguments. `startOpencodeServer` and `startExactOpencodeServer` collapse to one path since "exact" is now the only mode; keep both names exported if callers differ, or fold and update callers. In the fixture, `getExactNpmCacheDir`, its `NPM_CONFIG_CACHE` / `npm_config_update_notifier` overrides, and `prewarmExactOpencode` are removed: `bunx` caches under `$TMPDIR` per user and package regardless of `HOME` (measured), and the fixture forwards the real `TMPDIR`, so there is nothing to prewarm or forward there. In the eval runner, `buildEvalChildEnv`'s `NPM_CONFIG_CACHE` stays (the installed-artifact path's `npm install` and `npm pack` use it) and every child additionally receives `BUN_INSTALL_CACHE_DIR` as described under Key Technical Decisions. The cache directory is resolved by a small function in the availability helper so the fixture, the eval runner, and the eval-runner test module's probe all name the same directory. Because it is a stable path under a shared `os.tmpdir()`, it follows the same rule `bunx` applies to its own directory: the name carries the current uid and the pin (`systematic-bun-install-cache-<uid>-<pin>`), the resolver runs `mkdir` (recursive, mode `0700`) first and unconditionally — it succeeds silently on an existing directory or a symlink to one, so no "did it pre-exist" branch is needed — then always checks the path with `lstat` (never `stat`, which would follow a planted symlink to a directory the user does own): a symlink, a non-directory, or a directory not owned by the current uid is refused with a clear error, and a directory that is owned but wider than `0700` is tightened with `chmod` before use — cache hits are not re-verified against the registry, so a directory another user could pre-populate or redirect must never be trusted. The resolver is a plain per-call function like the probe, so the unit test can exercise creation and refusal in one process. The uid checks are POSIX-scoped: where `process.getuid` is undefined (Windows), the name uses `os.userInfo().username` and the ownership check is skipped, since `lstat` reports uid `0` for everything there; CI is `ubuntu-latest` and unaffected, and the Windows path exists only so `bun test tests/unit` keeps passing on a platform the tree already accommodates. It is never removed by the tooling; each pin adds about 200 MB of content-addressed tarballs, the pin in the name makes stale generations identifiable, and that growth on a contributor machine is accepted until the OS clears its temp root. It is passed as an env value, never stored on the `EvalFixture` struct: `fixtureControlledPaths` feeds `assertEvalPathContained`, and a process-scoped path there would trip `eval-path:path_escape`. The dogfood suite's `beforeAll` loses its prewarm call; its 360 s budget was mostly `packTarballOnce()`'s cold `bun run build` plus `npm pack`, not the prewarm, so it is re-derived from a measured run under Unit 4's job (where the build is already warm), not by subtracting the prewarm.
...
- `OPENCODE_AVAILABLE` becomes `isOpencodeAvailable()`, a memoizing function, at every existing `describe.skipIf` site, plus an exported `opencodeAvailabilityReason()` string. Each integration module's `skipIf` description or a `console.warn` carries the reason so a skipped local run is legible. The fixture module's own scope does no probing and no throwing, so its unit-side importers stay free of launcher fetches and of the flag.
- The helper's second export takes a classification and throws with its reason when `process.env.SYSTEMATIC_REQUIRE_OPENCODE === '1'` and the classification is not `available`. Each integration module that gates on the host calls it at its own module scope right after `isOpencodeAvailable()` — the five fixture-consuming suites and `eval-runner.test.ts` — never the fixture module itself, so the throw cannot fire during `bun test tests/unit` regardless of file order or the flag's in-process toggling in the helper's unit test. This is the CI fail-closed path, and it lives in one place so one fake-launcher unit test covers it for every consumer. `eval-runner.test.ts` has no fixture at module scope, so its probe env is a minimal explicit map — the parent's `PATH` (so `bunx` resolves), `HOME`, the `XDG_*` roots, and `TMPDIR` pointed at a throwaway `mkdtempSync` root, and `BUN_INSTALL_CACHE_DIR` from the helper's resolver — so the gate shares the one download rather than paying its own; the probe root is removed afterwards.
...
- Error path: the cache-directory resolver, pointed at a pre-existing path owned by another uid (simulated with a stubbed `lstatSync`), throws a clear error and never returns the path; pointed at a symlink, likewise; on a fresh temp root it creates the directory with mode `0700`; on a reused directory left at `0755` it returns the path with mode tightened to `0700`. These cases run in one process, so the resolver must not memoize.
...
- **State lifecycle risks:** `bunx` unpacks from the tooling-owned `BUN_INSTALL_CACHE_DIR`; a cold runner pays the download into it once per job, and a contributor machine once per pin. Orphaned hosts are covered by #896's group reaping; the probe's own child is short-lived.
...
| A future integration suite gates on `isOpencodeAvailable()` but forgets the throwing call | Locally nothing changes (it skips). In CI the suite's tests are skipped, and Unit 4's skip-set guard catches it: its skipped cases are not in the exempt set, so the job is red. |
...
| `@opencode-ai/sdk` and the `opencode-ai` CLI are separate npm packages; an sdk release without a matching CLI release makes `bunx opencode-ai@<pin>` unresolvable in the job that gates `release` | Inherited, not introduced: the current lockstep already spawns `opencode-ai@<sdk version>`. The probe classifies it `unavailable` and the job fails closed with a reason naming the version, so the failure is loud and attributable; the fix is to hold the Renovate group PR until the CLI version exists, which is a human step R6 does not promise away. |
