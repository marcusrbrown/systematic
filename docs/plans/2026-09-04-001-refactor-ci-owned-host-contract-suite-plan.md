...
- Error path: the cache-directory resolver, pointed at a pre-existing path owned by another uid (simulated with a stubbed `statSync`), throws a clear error and never returns the path; on a fresh temp root it creates the directory with mode `0700`.
