...
- **Fixed max turns = 20** (`MAX_DELEGATE_TURNS`), not configurable. Enforced via `AgentSession.subscribe()` counting real `turn_start` events; on exceeding the cap the tool calls `session.abort()` before a 21st turn can start, and returns `isError: true` with `outcome: 'turn_limit'` rather than hanging.
- **Fixed concurrency = 1** via the real `ToolDefinition.executionMode: 'sequential'` — no custom semaphore, no future parallel-mode surface.
- **Model inheritance is fail-closed, not fallback.** The tool passes `ctx.model` straight through; if `ctx.model` is `undefined`, the tool returns `isError: true` before any child session is created rather than letting Pi select a default model. `cwd` is pinned to `ctx.cwd` with no cwd input exposed. (Note: `noExtensions` prevents project-extension re-entry; it does not confine an explicitly allowed `bash` tool to that `cwd` — that is a real, separate blast-radius property of the `bash` tool itself, not claimed here.)
- **Ephemeral lifecycle:** `SessionManager.inMemory()`; the tool's own `AbortSignal` is propagated to `session.abort()` via an `abort`-event listener (re-checked immediately after the session is constructed, so a signal that aborts while `createDelegateSession()` is still pending is not missed); the `subscribe()` listener is unsubscribed and `session.dispose()` is always called in `finally` — covering prompt failures and the abort/turn-limit paths, and creation failures where a session exists (there is nothing to dispose if construction itself never returned a session). `abort()` is called at most once per run, its promise is awaited before the outcome is finalized, and a rejecting `abort()` is captured (never left unhandled) and reported as `failed` with the real turn count rather than a false clean `aborted`. A `prompt()` rejection caused by the tool's own abort request is treated as expected and does not override the authoritative `turn_limit`/`aborted` outcome; a `prompt()` rejection with no abort in flight is reported as `failed` with the actual turn count.
...
- **Result contract.** The tool returns the final assistant text (`session.getLastAssistantText()`) in `content`; `details` is a small discriminated object `{ persona, turnCount, outcome }` where `outcome` is `'completed' | 'turn_limit' | 'aborted' | 'failed'`. No usage/cost/UI-renderer fields are in scope.
...
- Test: `tests/unit/agent-resolver.test.ts` (new), `tests/unit/pi-delegate-tool.test.ts` (new), `tests/unit/pi-delegate-session.test.ts` (new — pins the live adapter's exact option/order contract without a provider), `tests/unit/pi.test.ts` (extended for `systematic_delegate` registration shape/description)
...
- Unknown persona and undefined parent model both fail closed *before* `createDelegateSession` is ever called (asserted via a spy flag in tests).
- 20-turn bound: a fake session emitting >20 `turn_start` events is aborted before the 21st turn, returns `isError: true` with `outcome: 'turn_limit'` and exactly `turnCount: 20`, and does not hang; `abort()` is called at most once.
- A signal already aborted before execution fails closed without constructing a child session.
- A signal that aborts while `createDelegateSession()` is still pending never calls `prompt()`, still aborts exactly once, disposes, and reports `outcome: 'aborted'`.
- A `prompt()` rejection caused by the turn-limit (or external) abort request still reports the authoritative `turn_limit`/`aborted` outcome, not a generic `failed`.
- A rejecting `abort()` (after either a turn-limit or an external abort request) is captured without an unhandled rejection and reported as `failed` with the real turn count, not a false clean abort.
- `prompt()` failing after several turns already ran preserves the actual `turnCount` in the `failed` result instead of reporting 0.
- Parent/tool `AbortSignal` abort propagates to `session.abort()` and returns `outcome: 'aborted'`.
...
**Verification:** `bun test tests/unit/agent-resolver.test.ts tests/unit/pi-delegate-tool.test.ts tests/unit/pi-delegate-session.test.ts tests/unit/pi.test.ts` green (54/54); full `bun test tests/unit` green (1067/1067); `bun run typecheck` clean; `bun run build` succeeds and a plain-Node `import('./dist/pi.js')` registers both tools; Biome clean on all changed files.
