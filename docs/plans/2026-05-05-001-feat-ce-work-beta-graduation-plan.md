...
- `~/.cache/opencode/packages/oh-my-opencode-slim@1.0.6/.../dist/index.js:23420-23456` — reference for `task` tool's actual argument contract: `{ subagent_type, description, prompt, task_id? }`.
...
- Modify: root `AGENTS.md` (skill count `46 → 45` at line 50, agent count `50 → 51` at line 51)
- Possibly modify: `docs/AGENTS.md` and `src/lib/AGENTS.md` if they carry duplicated counts (verify during execution)
...
- Update `AGENTS.md` counts: skills `46 → 45`, agents `50 → 51`. Verify with `find skills -name SKILL.md | wc -l` (should print 45) and `find agents -name '*.md' | wc -l` (still 50 at this point — Unit 2 brings it to 51; the count update runs in Unit 2's commit instead, after the agent file lands).
...
- `git status` shows clean diff: deleted `skills/ce-work-beta/`, modified `registry/registry.jsonc`, deleted `docs/src/content/docs/reference/skills/ce-work-beta.md`, modified `dist/registry/*`, modified `AGENTS.md` skill count.
...
Step 3 — update `AGENTS.md` agent count `50 → 51`. Verify with `find agents -name '*.md' | wc -l` (should print 51 after Step 1 added the file).
...
- `find agents -name '*.md' | wc -l` prints 51; AGENTS.md line 51 reads "51 bundled agents".
...
- **Reference implementation:** `~/.cache/opencode/packages/oh-my-opencode-slim@1.0.6/.../dist/index.js:23420-23456` (task tool argument contract).
