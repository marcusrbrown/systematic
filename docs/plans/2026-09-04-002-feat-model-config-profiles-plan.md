...
- R1–R3c. Per-harness `opencode`/`pi` blocks on agent and category overlays; flat fields as neutral default; block beats flat; agent beats category; qualifier-requires-model checked on the effective post-merge overlay; `model: null` permitted.
...
- **Profile selection is a load-time step between stripping and merging.** After each source is parsed and project's protected fields are stripped, the selector is resolved (custom > project > user), the named bundle is looked up in the user source's `profiles`, and the overlay chain becomes four entries: user base, selected profile, project, custom. `mergeOverlaySources` gains no new semantics — it merges a longer chain. Rationale: R10's order falls out of chain position; nothing else in the loader changes.
...
- Error path: `agents.x.pi.thinking` with no model at any layer → error naming `x` and `pi`.
